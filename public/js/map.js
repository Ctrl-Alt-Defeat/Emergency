...
function drowLocation() {
    let allLocationMap = otherLocations.map(ele => {
        ...
        return {
            ...
            'properties': {
                'description':
                    `<p><h1>This is ${ele.username}<h1/><h5>He is ${workStatus} Now</h5><h4>Years Of Experience: ${ele.exp}</h4><h4>Type Of Work: ${ele.type_of_work}</h4><h4>Contact And Read Or Add Feedback: <a href="/login/acconut/${ele.id}?is_not_enable=${true}">${ele.full_name}</a>.<h4/></p>`,
                'icon': 'embassy'
            },
            'geometry': {
                'type': 'Point',
